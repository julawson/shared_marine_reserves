...
colnames(results_matrix_so) <- c("timestep", "x_a_init", "x_a", "e_star_a", "x_b_init", "x_b", "e_star_b", "NPV_a_star", "NPV_b_star")
...
for(i in seq_along(x_a_grid)) {
  ...
  # Save results including e_hat_a and e_hat_b
  results_matrix_so[row_counter, ] <- c(t, x_a_init, x_a, e_star_values$e_star_a, x_b_init, x_b, e_star_values$e_star_b, NPV_a_star, NPV_b_star)
  row_counter <- row_counter + 1
...
matrix_df = dcast(df_e_a_star_interpolated, x_a ~ x_b, value.var = "e_a_star")
...
results_nar <-  results %>% 
  filter(accept > -0.05 & accept < 0.05)
...
ggplot() +
  geom_point(data = results, aes(x = x_b_init, y = accept))
