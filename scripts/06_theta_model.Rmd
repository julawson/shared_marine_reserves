...
num_rows <- length(x_a_grid) * length(x_b_grid) * timesteps * length(seq(0.1, 0.4, by = 0.1))
num_cols <- 10  
...
for(M_ab in seq(0.1, 0.4, by = 0.1)) {
...
grid_df <- expand.grid(x_a = seq(small, k_a, length.out = 100), x_b = seq(small, k_b, length.out = 100))
...
size_x_a = size_x_a #size of the state grid A
size_x_b = size_x_b #size of B
...
rownames(matrix_df) = matrix_df$x_a 
...
results_filter <- results %>% filter(accept >= -0.5 & accept <= 0.5)

```

```{r}

#write.csv(results, here("data","outputs","theta_Mab.csv"), row.names=FALSE)
