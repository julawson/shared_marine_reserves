...
df_results_non_bind = df_results_non %>% select(-x_a, -x_b, -h_a, -h_b) %>%  rename (x_a = x_a_init) %>% rename (x_b = x_b_init)
df_results_so_bind = df_results_so %>% select(-x_a, -x_b, -h_a, -h_b) %>% rename (x_a = x_a_init) %>% rename (x_b = x_b_init)
df_results_ea_bind = df_results_soa %>% select(-x_a, -x_b, -h_a, -h_b, -NPV_eb_star) %>% rename (x_a = x_a_init) %>% rename (x_b = x_b_init)

# Merging e_a_star values from df_inf into results based on x_a and x_b
results <- df_results_non_bind %>% left_join(df_results_so_bind, by= c("x_a", "x_b")) %>% 
  left_join(df_results_ea_bind, by= c("x_a", "x_b")) %>% 
  mutate(WTP_A = NPV_ea_star-NPV_a_hat,
         WTA_B = NPV_b_hat) %>% 
  mutate(accept = WTP_A-WTA_B) %>% 
  mutate(reserve_change = (NPV_a_star + NPV_b_star) - NPV_ea_star)
